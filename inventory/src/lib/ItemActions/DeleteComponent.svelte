...
<Confirm let:confirm={confirmThis} confirmTitle="Delete">
    <div class="delete-container" on:click={() => confirmThis(deleteItem, row)}>
        <span title={"Delete " + row.name} class="delete"
            ><Fa class="trash-can" icon={faTrashCan} /></span
        >
    </div>
    ...
    <span slot="description"> This can not be undone! </span>
</Confirm>
